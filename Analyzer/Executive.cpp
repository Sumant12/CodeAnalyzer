...
#include "../NoSQL/PersistHelp.h"
#include "../NoSQL/XmlDocument.h"
#include "../NoSQL/XmlElement.h"
#include "../NoSQL/XmlElementParts.h"
#include "../NoSQL/XmlParser.h"
#include "../NoSQL/Tokenizer2.h"
#include "../NoSQL/itokcollection2.h"
...
void PrintDB(NoSqlDb<StrData> &db) {
	...
	{
		std::cout << "\n  " << key << ":";
		std::cout << db.value(key).showElement();
	}
	std::cout << "\n\n";
...
void AddAllElementsToDb(Graph<std::string, std::string> g, NoSqlDb<StrData> &db) {
	typedef Graph<std::string, std::string> graph;
	typedef Vertex<string, string> vertex;
	stringstream ss = gettime();
	...
	while (iter != g.end())
	{
		...
		elem1.name = (v.value()).c_str();
		elem1.category = "Dependecy Category";
		elem1.data = "Dependecny Data";
		elem1.description = "Dependency Desc";
		elem1.timeDate = ss.str();
...
void Persist(NoSqlDb<StrData> db,string filename) {

	std::cout << "\n ========================================================";
	std::cout << "\n  Persisiting the noSqlDb data into xml document " <<filename << endl;
	std::cout << "\n ========================================================\n";
	DBWrapper dbwrap(db);
	string xmlsaved = dbwrap.save();
	std::ofstream ofs;
	//string str1 = "xmlpersisss.xml";
	...
	if (ofs.is_open())
	{
		...
		Utilities::StringHelper::title("The Db has been sucessfully persisted into the Database");
	}
	else {
...
void ConstructNoSqlDbforgraph(Graph<std::string, std::string> g,string filename) {
	NoSqlDb<StrData> db(100);



	AddAllElementsToDb(g, db);
	typedef Graph<std::string, std::string> graph;
	typedef Vertex<string, string> vertex;
	//typedef std::pair<int, E> edge;
	graph::iterator iter = g.begin();
	...
	{
		vertex v = *iter;
		Element<string> ele1 = db.value((v.value()).c_str());

		for (size_t i = 0; i<v.size(); ++i)
		{
			vertex::Edge edge = v[i];
	
			Element<string> elem2 = db.value(g[edge.first].value().c_str());
			db.AddChildren(ele1.name, elem2);

		}
		++iter;
	}

	Utilities::StringHelper::title("The Dependency Analysis information is pushed to the NoSqlDB ad follows: ");
	PrintDB(db);
	//string file = "persistdep.xml";
...
void ConstructNosqlfromMap(std::unordered_map<std::string, std::set<std::string>> depmap,string filename1) {
	NoSqlDb<StrData> db(100);
	stringstream ss = gettime();
	//AllElementsFromMap(depmap, db);
	for (std::unordered_map<string, set<string>>::iterator it = depmap.begin(); it != depmap.end(); ++it) {
		string name = it->first;
		set<string> vec = it->second;
		set<string>::iterator it1;
		Element<StrData> elem1;
		elem1.name = (name);
		elem1.category = "SrongComponents";
		elem1.data = "StringData";
		elem1.description = "StrongDesc";
		elem1.timeDate = ss.str();
	...
	Utilities::StringHelper::title("The Strong Components Analysis information is pushed to the NoSqlDB ad follows: ");
	PrintDB(db);
	//string file = "persistscc.xml";
	Persist(db, filename1);
...
void Requirement912() {
	std::cout << "\n ========================================================\n";
	std::cout << "\n  Requirement9 Demonstration: This Test suite demosntrates all the requirements.";
	std::cout << "\n ========================================================\n";
	std::cout << "\n ========================================================\n";
	std::cout << "\n  Requirement2 Demonstration: use the C++ standard library's streams for all I/O and new and delete for all heap-based memory management";
	std::cout << "\n ========================================================\n";
	std::cout << "\n ========================================================\n";
	std::cout << "\n  Requirement3 Demonstration: Each requirement has seperate package. Type Analysis,DependencyAnalayis and Strong component Analsis are in seperate packages";
	std::cout << "\n ========================================================\n";
}

void DependencyAnal(int argc,char* argv[]) {
	DependAnal dan;
	dan.ProcessCommandLine(argc, argv);
	dan.getSourceFiles();
	dan.processSourceCode();

	Utilities::StringHelper::Title("Dependency Analysis Completed");
	std::cout << "\n ========================================================\n";
	std::cout << "\n  Requirement5 Demonstration: Dependency Analysis has been done and the follwoing graph demonstrates that:";
	std::cout << "\n ========================================================\n";
	dan.displayDependencyGraph();

	std::unordered_map<std::string, std::set<std::string>>& depndencymap_ = dan.getDependencyTable();


	Graph<std::string, std::string> graph = dan.getDependGraph();
	//ConstructNoSql(depndencymap_);

	cout << endl << endl;
	std::cout << "\n ========================================================\n";
	std::cout << "\n  Requirement8 Demonstration: Path to the directory tree containing files to analyze is provided" << endl;
	std::cout << "\n  Requirement8 Demonstration: List of file patterns to match for selection of files to analyze. is provided" << endl;
	std::cout << "\n  Requirement8 Demonstration: If the files are not provided, the data is stored to default files. Here default files are defaultpersistdep.xml and defaultpersistscc.xml " << endl;
	std::cout << "\n ========================================================\n";
	string filename;
	if (argc <= 6) {
		filename = "defaultpersistdep.xml";
	}
	else {
		filename = argv[6];
	}
	ConstructNoSqlDbforgraph(graph, filename);
}

void StrongComponents(int argc,char* argv[]) {
	StrongComp sc;
	sc.SCC();

	std::cout << "\n ========================================================\n";
	std::cout << "\n  Requirement6 Demonstration: Strong Component Analysis has been done. Check the xml for Strong component information:";
	std::cout << "\n ========================================================\n";
	unordered_map<string, set<string>> map1;
	map1 = sc.getStrongMap();
	string filename1;
	if (argc <= 7) {
		filename1 = "defaultpersistscc.xml";
	}
	else {
		filename1 = argv[7];
	}

	ConstructNosqlfromMap(map1, filename1);
	std::cout << "\n ========================================================\n";
	std::cout << "\n  Requirement7 Demonstration: Dependency Analysis and Strong component Analysis has been stored in NoSql and persisted to xml";
	std::cout << "\n ========================================================\n";
}
...
{	
  ...
  try {
	...
	std::cout << "\n ========================================================\n";
	std::cout << "\n  Requirement4 Demonstration: Type Table has been constructed and it is as follows:";
	std::cout << "\n ========================================================\n";
    TypeAnal ta;
    ta.doTypeAnal();
	ta.displayTypeTable();
	...
	DependencyAnal(argc, argv);
	StrongComponents(argc, argv);
