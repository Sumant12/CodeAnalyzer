...
Toker2::Toker2(const std::string& src, bool isFile)
...
void Toker2::setMode(Toker2::mode md)
{
	_mode = md;
	scTok = "()[]{};.\n";
	if (_mode == xml)
...
void Toker2::setSingleCharTokens(std::string tokChars)
{
	_mode = custom;
	scTok = tokChars;
}
...
bool Toker2::attach(const std::string& name, bool srcIsFile)
{
	if (pIn && srcIsFile)
	{
		pIn->clear();
...
bool Toker2::isSingleCharTok(char ch)
{
...
bool Toker2::isIdentifierChar(char ch)
{
...
int main(int argc, char* argv[])
{
	...
	Toker2 t_fromStr("tokens from a string: -> int x; /* a comment */", false);
	std::string tok;
	do
	{
	...
	for (int i = 1; i<argc; ++i)
	{
		...
		{
			Toker2 t;
			t.setMode(Toker2::xml);        // comment out to show tokenizing for code
