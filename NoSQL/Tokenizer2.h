...
"(", ")", "{", "}", "[". "]", ";", ".", and "\n".
You can change the selection with Toker2::setMode(Toker2::code) or
Toker2::setMode(Toker2::xml), or Toker2::setSingleCharTokens(somestring);
- Toker2::code is the default tokenizing mode.
A tokenizer is an important part of a scanner, used to read and interpret
source code or XML.

Public Interface:
=================
Toker2 t;                        // create tokenizer instance
...
public:
	...
	Toker2(const std::string& src = "", bool isFile = true);
	~Toker2();
...
private:
	...
	Toker2(const Toker2 &tkr);
	Toker2& operator=(const Toker2&);
};

inline void Toker2::returnComments(bool doReturn)
{
	doReturnComments = doReturn;
}

inline bool Toker2::isFileEnd() { return (nextChar == -1); }

inline int& Toker2::lines() { return numLines; }

inline int Toker2::braceLevel() { return braceCount; }
